...
class Movie extends Asset {
  ...
  public function trailer($id){
    $trailers = "";
    $db = TMDBV3::getInstance();
    $info = $db->info(self::$type, $id, 'trailers');
    if (empty($info->youtube[0]))
        return "";
    return "http://www.youtube.com/watch?v=".$info->youtube[0]->source;
  }


    public function director_list($id){
    $directors = "";
    $db = TMDBV3::getInstance();
    $info = $db->info(self::$type, $id, 'casts');
    foreach($info->crew as $persons) {
        if ($persons->job == "Director")
            $directors .= $persons->name . ",";
    }
    return $directors;
  }

  public function cast_list($id){
    $casts = "";
    $db = TMDBV3::getInstance();
    $info = $db->info(self::$type, $id, 'casts');
    foreach($info->cast as $persons) {
        $casts .= $persons->name . ",";
    }
    return $casts;
  }
